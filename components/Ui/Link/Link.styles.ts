...
export const Button = styled.a<IButton>`
  ...
  background-position-x: ${({ backgroundPositionX }) =>
    backgroundPositionX
      ? `${
          isNaN(Number(backgroundPositionX))
            ? backgroundPositionX
  ...
  background-position-y: ${({ backgroundPositionY }) =>
    backgroundPositionY
      ? `${
          isNaN(Number(backgroundPositionY))
            ? backgroundPositionY
  ...
  background-size: ${({ backgroundSize }) =>
    backgroundSize
      ? `${
          isNaN(Number(backgroundSize)) ? backgroundSize : `${backgroundSize}px`
        } !important`
      : "auto"};
  border: none;
  border-radius: 1rem;
  box-shadow: ${({ shadowColor, shadowed }) =>
    shadowed
      ? `0 .5rem 1rem ${
          shadowColor ? makeShadowColor(shadowColor, 0.5) : "rgba(0,0,0,.15)"
        }!important`
      : "initial"};
  ...
  position: relative;
  top: 0;
  transition: box-shadow 0.15s, top 0.15s;
  width: ${({ width }) =>
    width
  ...
  &:hover {
    box-shadow: ${({ shadowColor, shadowed }) =>
      shadowed
        ? `0px 0px 0.75rem 0.125rem ${
            shadowColor ? makeShadowColor(shadowColor, 0.85) : "rgba(0,0,0,.75)"
          }!important`
        : "initial"};
    top: -5px;
  }
